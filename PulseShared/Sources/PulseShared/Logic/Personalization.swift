...
public struct Personalization {
    ...
    ///
    /// The running sum of those steps is averaged (per correction) then clamped to
    /// `[-0.3, 0.3]`. Note: when all corrections agree (e.g. 50 positive-direction
    /// corrections), the raw sum can exceed the clamp bounds — the clamp is what
    /// drives the test expectation of 0.3 for a large uniform batch.
    ///
